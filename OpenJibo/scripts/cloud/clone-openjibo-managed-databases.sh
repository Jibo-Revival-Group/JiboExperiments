...
remove_rule() {
  ...
  local server_name="$2"
  az postgres flexible-server firewall-rule delete     --resource-group "$resource_group"     --server-name "$server_name"     --rule-name "$rule_name"     --yes     --output none >/dev/null 2>&1 || true
}
...
create_rule() {
  local resource_group="$1"
  local server_name="$2"
  az postgres flexible-server firewall-rule create     --resource-group "$resource_group"     --server-name "$server_name"     --rule-name "$rule_name"     --start-ip-address "$runner_ip"     --end-ip-address "$runner_ip"     --output none >/dev/null
}
