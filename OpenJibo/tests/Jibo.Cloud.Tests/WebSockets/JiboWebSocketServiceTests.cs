...
public sealed class JiboWebSocketServiceTests
{
    ...
    public async Task BufferedHotphraseOggAudio_DoesNotFinalizeOnOneSecondMidStreamGap()
    {
        ...
        await _service.HandleMessageAsync(new WebSocketMessageEnvelope
        {
            ...
            Token = "hub-hotphrase-ogg-silence-token",
            Text =
                """{"type":"CONTEXT","transID":"trans-hotphrase-ogg-silence","data":{"audioTranscriptHint":"what's your cloud version"}}"""
        });
        ...
        var session = _store.FindSessionByToken("hub-hotphrase-ogg-silence-token");
        Assert.NotNull(session);
        session.TurnState.FirstAudioReceivedUtc = DateTimeOffset.UtcNow - TimeSpan.FromSeconds(5);
        session.TurnState.LastAudioReceivedUtc = DateTimeOffset.UtcNow - TimeSpan.FromMilliseconds(1000);
    ...
    public async Task ClientAsr_HowIsTheWeather_EmitsSpokenWeatherFallbackWithoutRedirect()
    {
        ...
        Assert.Equal(JsonValueKind.Null,
            listenPayload.RootElement.GetProperty("data").GetProperty("match").GetProperty("cloudSkill").ValueKind);
    ...
    public async Task BufferedHotphraseOggAudio_EarlyProbeFinalizesUsableCommandBeforeHardTimeout()
    {
        ...
        Assert.NotNull(session);
        session.TurnState.FirstAudioReceivedUtc = DateTimeOffset.UtcNow - TimeSpan.FromMilliseconds(5800);
        session.TurnState.LastAudioReceivedUtc = DateTimeOffset.UtcNow - TimeSpan.FromMilliseconds(1050);
