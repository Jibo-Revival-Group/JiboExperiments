...
$command = "cd '$(Escape-BashSingleQuoted $scriptDir)' && ./build-linux-filesystem-from-copies.sh --source-root '$(Escape-BashSingleQuoted $sourceRootUnix)' --output-root '$(Escape-BashSingleQuoted $outputRootUnix)'"
if ($Clean) {
    $command += " --clean"
}
