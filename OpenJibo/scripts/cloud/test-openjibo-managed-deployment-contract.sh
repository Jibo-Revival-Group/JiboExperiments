...
for marker in "deployment_target" "openjibo-staging-gate" "clone-openjibo-managed-databases.sh" "production_backup_confirmed" "backup.backupRetentionDays" "properties.active" '[[ "$revision_active" == "true" ]]' "revision deactivate" "Restore previous image after failure"; do
  if [[ "$workflow_text" != *"$marker"* ]]; then
...
for marker in "pg_dump" "pg_restore" "firewall-rule create" "firewall-rule delete" '--server-name "$server_name"' "Source and target resource groups must be different" "source and target PostgreSQL hosts are identical"; do
