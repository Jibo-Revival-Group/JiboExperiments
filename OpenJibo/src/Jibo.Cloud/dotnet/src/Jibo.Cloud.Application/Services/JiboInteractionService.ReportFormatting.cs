...
public sealed partial class JiboInteractionService
{
    ...
    private static IDictionary<string, object?> BuildCalendarSkillPayload(CalendarReportSnapshot? snapshot)
    {
        ...
        payload["calendar_event_icon"] = ResolveCalendarEventIcon(summary);
        return payload;
    }

    // Mirrors Pegasus report-skill calendarIconWords.json. Defaults to the always-present
    // "calendar" icon so the GUI never references a missing texture (which hangs the display).
    private static readonly IReadOnlyList<(string Icon, string[] Keywords)> CalendarIconKeywords =
    [
        ("apple", ["apple", "fruit"]),
        ("airplane", ["airplane", "flight", "airport"]),
        ("ambulance", ["hospital", "ambulance"]),
        ("baby", ["baby", "infant", "newborn"]),
        ("bandaid", ["bandaid", "band-aid", "band aid"]),
        ("bank", ["bank", "money", "currency", "finance"]),
        ("barbell", ["barbell", "fitness", "gym"]),
        ("baseball", ["baseball"]),
        ...
        ("beer", ["beer"]),
        ("bicycle", ["bicycle", "cycling", "bicycling", "bike"]),
        ("birthdaycake", ["birthday", "cake"]),
        ("book", ["book", "novel"]),
        ("breakfast", ["breakfast", "egg", "eggs", "brunch"]),
        ("camera", ["camera", "photo"]),
        ("camping", ["camp", "camping", "tent"]),
        ("car", ["car", "vehicle", "auto", "automobile"]),
        ("cat", ["cat"]),
        ("chalkboard", ["chalkboard", "education", "teaching", "learning"]),
        ("christmas", ["christmas"]),
        ("cincodemayo", ["cincodemayo", "cinco de mayo"]),
        ("city", ["city", "urban", "building", "skyscraper"]),
        ("clock", ["clock", "time"]),
        ("coffee", ["coffee", "tea", "cafe", "cocoa"]),
        ("computer", ["computer", "laptop", "desktop"]),
        ("dinner", ["dinner", "meal"]),
        ("discoball", ["party", "dance", "dancing"]),
        ("dog", ["dog", "animal"]),
        ("earth", ["earth", "world"]),
        ("easter", ["easter"]),
        ...
        ("groceries", ["groceries"]),
        ("hamburger", ["burger"]),
        ("hanukkah", ["hanukkah", "chanukkah"]),
        ("heart", ["heart", "love", "valentine"]),
        ("house", ["house", "home"]),
        ("icecream", ["ice cream", "icecream", "cone", "sundae"]),
        ("kwanzaa", ["kwanzaa"]),
        ("laundry", ["laundry"]),
        ("lightning", ["lightning"]),
        ("mountains", ["mountain"]),
    ...
    private static string ResolveCalendarEventIcon(string? summary)
    {
        if (string.IsNullOrWhiteSpace(summary)) return "calendar";

        foreach (var (icon, keywords) in CalendarIconKeywords)
            if (keywords.Any(keyword =>
                    summary.Contains(keyword, StringComparison.OrdinalIgnoreCase)))
                return icon;

        // Default matches Pegasus getIconImgName fallback.
        return "calendar";
    }
